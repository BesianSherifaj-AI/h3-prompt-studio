...
import { ensurePromptTags,renamePromptTag,replacePhoto,movePhoto } from './tags';
import { newShot, type Project, type Asset } from './model';
...
describe('stable photo references',()=>{
  ...
  it('keeps URL path tags and escaped literal handles unchanged',()=>{const p=make();ensurePromptTags(p);p.story.text='https://example.com/@green-dress \\@green-dress @green-dress';renamePromptTag(p,'a','emerald');expect(p.story.text).toBe('https://example.com/@green-dress \\@green-dress @emerald');});
  it('replacement preserves ownership, role and tag, rebinds identity, and discards old image captions',()=>{const p=make();ensurePromptTags(p);p.assets[0].simple_owner_id='s';p.simple={previous_image_roles:{a:'reference_image'}};replacePhoto(p,'a',asset('new','Upload'));expect(p.subjects[0].asset_ids).toEqual(['new']);expect(p.assets[0]).toMatchObject({id:'new',prompt_tag:'green-dress',simple_owner_id:'s',approved_observation:'',observation:'',name:'Green Dress'});expect(p.simple.previous_image_roles).toEqual({new:'reference_image'});});
});
